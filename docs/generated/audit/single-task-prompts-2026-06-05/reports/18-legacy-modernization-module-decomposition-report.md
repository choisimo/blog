# 18. 레거시 시스템 현대화 모듈 분해 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/18-legacy-modernization-module-decomposition.md`
...
## 0. 레거시 위험 요약
- 판정: Legacy/compatibility surfaces remain: backend-origin APIs behind worker, legacy completions fallback, disabled WebSocket chat, old route aliases.
- 분석 초점: 레거시 현대화
...
## 1. 기능/데이터 경계 지도
- Public config sets `supportsChatWebSocket=false`; code throws for chat websocket transport.
- ai-worker logs show legacy completions fallback active for `gpt-5-mini`.
- Translation routes include legacy public alias tests.

## 2. Hidden Coupling 목록
- Public config sets `supportsChatWebSocket=false`; code throws for chat websocket transport.
- ai-worker logs show legacy completions fallback active for `gpt-5-mini`.
- Translation routes include legacy public alias tests.

권장 작업:
1. Define sunset criteria for legacy completions fallback.
2. Document old `/post/:year/:slug` and translation alias lifetime.
3. Remove compatibility only after telemetry confirms no traffic.
