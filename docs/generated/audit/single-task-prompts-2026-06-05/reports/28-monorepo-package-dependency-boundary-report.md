# 28. 모노레포/패키지 경계/의존성 분석 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/28-monorepo-package-dependency-boundary.md`
...
## 0. 모노레포 경계 요약
- 판정: Monorepo boundaries are partly formalized through `@blog/shared`, but route governance drift indicates boundary automation is not fully enforced.
- 분석 초점: 모노레포/패키지 경계
...
## 1. Package Inventory
- Packages: root scripts, `frontend`, `backend`, `workers/api-gateway`, `shared`, terminal server, other worker gateways.
- `@blog/shared` is consumed by frontend/backend/workers for contracts.
- Route drift check failed while contract drift passed.
...
권장 작업:
1. Make route governance required in CI after drift correction.
2. Keep shared contracts versioned and avoid importing app internals across packages.
3. Add dependency graph report excluding node_modules/generated public assets.
