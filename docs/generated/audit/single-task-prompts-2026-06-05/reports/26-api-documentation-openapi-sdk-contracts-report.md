# 26. API 문서화/OpenAPI/SDK 계약 생성 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/26-api-documentation-openapi-sdk-contracts.md`
...
## 0. API 문서화 요약
- 판정: Endpoint inventory exists but should not be treated as authoritative until route-governance drift is resolved.
- 분석 초점: API 문서/OpenAPI/SDK 계약
...
## 1. API Catalog
- `endpoint-map-full.csv` has 370 endpoint rows.
- Route boundary headers provide machine-readable owner/responder/edge mode metadata.
- Public metrics intentionally returns 404 through worker; direct backend metrics requires backend key.

권장 작업:
1. Generate OpenAPI from current route registry after drift fix.
2. Tag each endpoint as worker-owned/backend-owned/proxy-only/compatibility.
3. Create SDK only from public, stable endpoints, excluding admin/internal/metrics.
