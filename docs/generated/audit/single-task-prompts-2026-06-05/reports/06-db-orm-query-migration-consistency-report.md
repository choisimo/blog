# 06. DB/ORM/쿼리/마이그레이션 정합성 분석 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/06-db-orm-query-migration-consistency.md`
...
## 0. 데이터 정합성 요약
- 판정: 운영 DB 의존성은 healthy지만 schema ownership이 Postgres/D1/KV/R2/Redis/Chroma로 분산되어 변경 위험이 크다.
- 분석 초점: DB/쿼리/마이그레이션
...
- Worker cron deletes D1 `post_views` older than 90 days; backend readiness validates Postgres, Redis, D1, Chroma, worker, domain outbox.
- No actual restore/backfill drill was executed in this run.

## 권장 액션
1. D1/Postgres migration inventory와 rollback order를 한 문서로 합친다.
2. analytics canonical store(Postgres)와 D1 cache/editor-picks 책임을 명시한다.
3. schema change는 expand-contract와 snapshot tests로 통제한다.
