# 12. 운영/관측성/장애 대응 준비도 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/12-operations-observability-incident-readiness.md`
...
## 0. 운영 준비도 요약
- 판정: Readiness/metrics/logging 기반은 있으나 alert/runbook 연결과 cloudflared lifecycle 관리가 부족하다.
- 분석 초점: 운영/관측성/장애 대응
...
- Backend exposes `/metrics` behind backend key; worker intentionally returns 404 for public `/metrics`.
- Readiness includes processLocalState warning for single-instance mode.
...
## 권장 액션
1. SLO: readiness ok rate, API 5xx, queue pending/DLQ, translation job age, SSE disconnects를 정의한다.
2. cloudflared outdated warning and origin refused/DNS incidents에 alert/runbook을 연결한다.
3. domain_outbox deadLetter/stuck thresholds를 page-worthy alert로 만든다.
