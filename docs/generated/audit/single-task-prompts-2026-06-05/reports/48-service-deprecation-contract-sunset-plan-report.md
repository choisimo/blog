# 48. Service/API Deprecation Contract Sunset 계획 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/48-service-deprecation-contract-sunset-plan.md`
...
## Prompt Output
- Chat WebSocket capability is false and client code throws for removed WS transport.
- Legacy translation aliases and `/post/:year/:slug` route remain.
- AI legacy completions fallback is active in production logs.
## 핵심 Findings
- Chat WebSocket capability is false and client code throws for removed WS transport.
- Legacy translation aliases and `/post/:year/:slug` route remain.
- AI legacy completions fallback is active in production logs.
...
## 권장 액션
1. Create deprecation table: endpoint/feature, consumers, telemetry, sunset date, replacement.
2. Add response deprecation headers for legacy aliases where feasible.
3. Remove only after route telemetry shows no usage.
