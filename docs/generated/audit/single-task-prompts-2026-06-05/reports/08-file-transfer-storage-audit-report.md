# 08. 파일 업로드/다운로드 스트리밍 스토리지 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/08-file-transfer-storage-audit.md`
...
## 0. 파일 처리 위험 요약
- 판정: 이미지/markdown/static/R2 흐름은 존재하며 SVG rejection tests가 있다. 실제 업로드 E2E는 미실행.
- 분석 초점: 파일/스토리지
...
## 1. 파일 플로우 맵
- Frontend static markdown and manifests loaded from `noblog.nodove.com/posts...` with 200 responses.
- Backend image security tests reject SVG uploads and remote private image URLs.
- Worker images routes handle R2 direct upload/chat upload/delete, but production upload mutation was not exercised.

권장 작업:
1. R2 upload/delete를 staging bucket 또는 test namespace로 E2E 검증한다.
2. MIME sniffing, extension, size limit, path segment sanitization을 API docs에 표기한다.
3. object-storage orphan cleanup and rollback runbook을 추가한다.
