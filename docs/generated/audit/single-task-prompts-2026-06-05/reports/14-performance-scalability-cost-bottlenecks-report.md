# 14. 성능/확장성/비용 병목 분석 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/14-performance-scalability-cost-bottlenecks.md`
...
## 0. 성능 위험 요약
- 판정: 현재 관측 시 리소스는 낮지만 load test/throughput evidence는 없다. single-instance local state is scaling risk.
- 분석 초점: 성능/확장성/비용
...
## 3. 확장성 한계
- `kubectl top pods` showed api 4m CPU/94Mi, ai-worker 2m/30-31Mi, open-notebook/surrealdb higher memory.
- AI queue uses pending backlog rather than raw stream length for pressure; stream maxlen capped at 10000.
- Readiness exposes `processLocalState.mode=single-instance`.

권장 작업:
1. k6/Playwright synthetic load로 blog post, search, comments, AI/RAG read path를 측정한다.
2. SSE/live room and in-memory session state의 horizontal scaling readiness를 검증한다.
3. AI fallback latency and token cost budget을 dashboard에 추가한다.
