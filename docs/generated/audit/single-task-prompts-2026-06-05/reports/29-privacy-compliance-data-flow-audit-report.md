# 29. 개인정보/컴플라이언스 데이터 흐름 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/29-privacy-compliance-data-flow-audit.md`
...
## 0. 개인정보 위험 요약
- 판정: PII-like surfaces exist: auth tokens, OAuth email allowlist, user IDs, comments, fingerprints/IP/rate-limit keys, logs, analytics.
- 분석 초점: 개인정보/컴플라이언스 데이터 흐름
...
## 1. PII Inventory
- Worker/backend logs include request IDs, methods, paths, statuses; Cloudflared masks tunnel token.
- Comments/rate-limit tests use fingerprint/IP/UA semantics.
- No legal-hold or DSAR runbook was verified.
...
권장 작업:
1. Create PII inventory with retention for comments, analytics, auth refresh tokens, user content, logs.
2. Ensure logs redact Authorization, cookies, setup tokens, API keys.
3. Define deletion/export flow for user content and comments.
