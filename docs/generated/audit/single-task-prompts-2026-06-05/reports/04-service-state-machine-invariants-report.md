# 04. 서비스별 상태 기계 불변식 상태 변화 조건 분석 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/04-service-state-machine-invariants.md`
...
## 0. 서비스 위험 요약
- 판정: 핵심 상태 기계는 식별됨: auth token family, AI queue, translation jobs, notification/outbox, comment lifecycle.
- 분석 초점: 상태 기계/불변식
...
- Readiness invariant: required dependencies failing should degrade readiness; tests cover degraded/required/optional dependency cases.
- Auth invariant: refresh token cannot act as access token; tests cover canonical issuer/audience and legacy token rejection.

권장 작업:
1. 각 state machine에 explicit state enum과 transition guard 문서를 추가한다.
2. outbox 실패 재시도/포기 조건과 DLQ threshold alert를 운영 runbook에 연결한다.
3. comments/reactions deletion 후 visibility invariant를 E2E로 보강한다.
