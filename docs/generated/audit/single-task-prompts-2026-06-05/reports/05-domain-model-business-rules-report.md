# 05. 도메인 모델 비즈니스 규칙 정규화 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/05-domain-model-business-rules.md`
...
## 0. 도메인 개요
- 판정: 도메인은 blog content, analytics, comments, auth/session, AI/RAG, admin publishing으로 분해된다.
- 분석 초점: 도메인 모델/비즈니스 규칙
...
## 4. 권장 도메인 경계
1. post publication/draft/translation/comment rules를 shared contract 또는 ADR로 정리한다.
2. analytics editor-picks scoring과 expiry를 제품 규칙으로 문서화한다.
3. feature flag별 user-visible behavior를 product contract로 연결한다.
...
## 핵심 Findings
- Public post visibility hides drafts by default; backend tests cover draft exclusion and admin inclusion.
- Runtime feature flags govern terminal/code execution/comments/AI/RAG behavior.
- Route ownership is treated as a domain-level contract, not only infrastructure metadata.
