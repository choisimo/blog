# 47. Scheduler/Cron Idempotency/Replay 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/47-scheduler-cron-idempotency-replay-audit.md`
...
## Prompt Output
- Worker scheduled handler signs backend refresh/editor-picks calls and continues non-fatally on errors.
- Backend domain outbox worker prevents overlapping runs with `running` flag.
- Origin-signature tests cover cron backend calls.
## 핵심 Findings
- Worker scheduled handler signs backend refresh/editor-picks calls and continues non-fatally on errors.
- Backend domain outbox worker prevents overlapping runs with `running` flag.
- Origin-signature tests cover cron backend calls.
...
## 권장 액션
1. Add manual replay commands for each scheduler phase.
2. Store scheduler run IDs and idempotency keys for refresh/editor-picks/outbox.
3. Alert on repeated non-fatal cron errors.
