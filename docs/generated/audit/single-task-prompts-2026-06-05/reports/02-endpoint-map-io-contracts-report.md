# 02. 전체 엔드포인트 맵 및 입출력 계약 추출 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/02-endpoint-map-io-contracts.md`
...
## 0. 누락 위험 요약
- 판정: 주의 필요: 370개 endpoint row가 추출되어 있으나 route-governance snapshot drift 때문에 공식 계약으로 단정하기 어렵다.
- 분석 초점: 엔드포인트 계약
...
- 공개 worker 응답은 `X-Route-Boundary-*` 헤더를 반환한다.
- 쓰기 경로는 analytics, comments, auth/session, user-content, memos, memories, AI, image, admin, translation job 쪽에 집중되어 있다.

## 권장 액션
1. `endpoint-map-full.csv`를 route registry 최신 상태로 재생성하거나 drift 원인을 수정한다.
2. OpenAPI/SDK 생성 전 worker proxy-only 경로와 backend native 경로를 분리한다.
3. 쓰기 endpoint에 idempotency/transaction column을 필수 필드로 유지한다.
