# 21. 데이터 파이프라인/배치/스케줄러 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/21-data-pipeline-batch-scheduler-audit.md`
...
## 0. 배치 위험 요약
- 판정: Cron/outbox/queue mechanisms exist and tests pass, but replay/backfill operational playbooks need completion.
- 분석 초점: 데이터 파이프라인/배치/스케줄러
...
## 1. Job Inventory
- Worker scheduled handler refreshes analytics, updates editor picks, cleans 90-day D1 views, flushes AI artifact and notification outboxes.
- Backend domain outbox worker runs every 30000ms by default.
- Origin-signature tests cover scheduled backend calls.
...
## 4. Backfill/Replay 계획
- Worker scheduled handler refreshes analytics, updates editor picks, cleans 90-day D1 views, flushes AI artifact and notification outboxes.
- Backend domain outbox worker runs every 30000ms by default.
- Origin-signature tests cover scheduled backend calls.

권장 작업:
1. Write replay/backfill commands for analytics and editor picks.
2. Add max job age/stuck outbox alerts.
3. Document idempotency for each scheduled side effect.
