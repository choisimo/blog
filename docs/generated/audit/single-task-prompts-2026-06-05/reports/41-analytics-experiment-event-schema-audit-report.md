# 41. Analytics/Experiment Event Schema 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/41-analytics-experiment-event-schema-audit.md`
...
## Prompt Output
- Production editor-picks/trending endpoints returned 200 with live data.
- Frontend heartbeats returned 200 during E2E; analytics view was aborted during navigation.
- Worker cron refreshes stats and editor picks; D1 cleanup retains 90 days.
## 핵심 Findings
- Production editor-picks/trending endpoints returned 200 with live data.
- Frontend heartbeats returned 200 during E2E; analytics view was aborted during navigation.
- Worker cron refreshes stats and editor picks; D1 cleanup retains 90 days.
...
## 권장 액션
1. Add event schema version and idempotency key to analytics view/heartbeat.
2. Create dashboard for failed/aborted analytics writes.
3. If experiments are introduced, keep assignment event separate from exposure/conversion.
