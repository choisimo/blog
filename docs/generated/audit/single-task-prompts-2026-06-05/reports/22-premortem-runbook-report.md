# 22. 장애 시나리오 Pre-mortem 및 Runbook 생성 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/22-premortem-runbook.md`
...
## 0. Pre-mortem 요약
- 판정: Most likely incidents: route drift deployment, Cloudflared/origin DNS issue, dependency CVE exposure, AI queue/provider degradation, translation/RAG background failures.
- 분석 초점: Pre-mortem/runbook
...
## 1. 장애 시나리오 표
- Kubernetes readiness reports all required dependencies ok; `kubectl top` showed low CPU/memory at test time; cloudflared logs show an outdated version warning and prior origin/DNS/connect-refused incidents.
- Cloudflared logs contain prior connect refused and DNS timeout events for origin service.
- Readiness currently healthy, so incident evidence is historical/recent log based, not current outage.
...
## 권장 액션
1. Runbook step 1: compare `/api/v1/readiness`, k3s pod status, and cloudflared logs.
2. Runbook step 2: inspect route boundary headers for failing endpoint.
3. Runbook step 3: check Redis queue/DLQ and domain_outbox readiness detail.
