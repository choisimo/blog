# 09. 외부 API/Webhook/Queue/Event 연동 안정성 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/09-external-api-webhook-queue-event-audit.md`
...
## 0. 연동 위험 요약
- 판정: 외부 연동은 작동하지만 degraded fallback이 많은 구조다: OpenAI-compatible AI, Chroma, Cloudflare, GitHub, Redis queue, outboxes.
- 분석 초점: 외부 API/Webhook/Queue/Event
...
## 1. 외부 연동 맵
- AI async work uses Redis Streams with consumer groups, retries, DLQ, max stream length, and queue stats; backend domain outbox periodically claims and marks durable events; worker cron flushes artifact and notification outboxes.
- AI health reports provider `openai-compat` healthy; ai-worker logs show legacy completions fallback used in production.
- cloudflared logs show prior origin refused/DNS timeout incidents and current version outdated warning.

권장 작업:
1. external calls별 timeout/retry/idempotency matrix를 endpoint map에 연결한다.
2. legacy completions fallback의 제거 조건과 alert를 정한다.
3. cloudflared upgrade and DNS/origin incident checks를 runbook에 넣는다.

## 2. Webhook/Consumer 맵
- AI async work uses Redis Streams with consumer groups, retries, DLQ, max stream length, and queue stats; backend domain outbox periodically claims and marks durable events; worker cron flushes artifact and notification outboxes.
- AI health reports provider `openai-compat` healthy; ai-worker logs show legacy completions fallback used in production.
- cloudflared logs show prior origin refused/DNS timeout incidents and current version outdated warning.
