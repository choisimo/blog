# 45. Config Drift/Environment Parity 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/45-config-drift-environment-parity-audit.md`
...
## Prompt Output
- `contracts:check` passes but `routes:check` fails.
- Production uses k3s Deployment/ConfigMap/Secret/cloudflared; local compose includes nginx/workers-local/terminal server specifics.
- Public config direct origin 401 differs from worker public 200 by design.
## 핵심 Findings
- `contracts:check` passes but `routes:check` fails.
- Production uses k3s Deployment/ConfigMap/Secret/cloudflared; local compose includes nginx/workers-local/terminal server specifics.
- Public config direct origin 401 differs from worker public 200 by design.
...
## 권장 액션
1. Regenerate/fix route governance snapshot.
2. Run redacted config inventory check against production ConfigMap keys.
3. Document intended parity gaps between local compose and k3s.
