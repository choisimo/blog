# 32. Dependency/SBOM/License/Supply Chain 감사 - 실행 보고서

- 원본 프롬프트: `/home/nodove/workspace/secret/프롬프트/프로젝트_작업_프롬프트_라이브러리/single-task-prompts/32-dependency-sbom-license-supply-chain-audit.md`
...
## Prompt Output
- Backend prod audit: 8 advisories, including high severity `express`, `path-to-regexp`, `jws` and moderate `uuid`, `ws`, `qs`.
- Frontend prod audit: 29 advisories, including critical `fast-xml-parser` and `protobufjs`, high router/lodash/minimatch/node-forge/picomatch classes.
- Tests still pass, so this is patch/risk-management work rather than functional breakage.
## 핵심 Findings
- Backend prod audit: 8 advisories, including high severity `express`, `path-to-regexp`, `jws` and moderate `uuid`, `ws`, `qs`.
- Frontend prod audit: 29 advisories, including critical `fast-xml-parser` and `protobufjs`, high router/lodash/minimatch/node-forge/picomatch classes.
- Tests still pass, so this is patch/risk-management work rather than functional breakage.
...
## 권장 액션
1. Patch backend express/jsonwebtoken/uuid/ws tree and rerun tests.
2. Patch frontend router/firebase/protobuf/XML parser chains; major updates need regression plan.
3. Generate SBOM and advisory allowlist with expiry only for unavoidable transitive issues.
