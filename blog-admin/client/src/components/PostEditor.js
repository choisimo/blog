...
function PostEditor() {
  ...
  const fetchPost = async () => {
    try {
      ...
      setPost({
        ...
        publishTime:
          response.data.publishTime || moment().format('YYYY-MM-DD HH:mm:ss'),
      });
      setHasUnsavedChanges(false);
    } catch (error) {
  ...
  const fetchOpenRouterModels = async (apiKey = null) => {
    try {
      ...
      const response = await axios.get('/api/ai/openrouter/models', {
        headers,
      });
  ...
  const generateAIContent = async () => {
    ...
    try {
      ...
      if (!apiSettings.apiKeys?.gemini && !apiSettings.apiKeys?.openrouter) {
        toast.error(
          'AI 설정이 필요합니다. 설정 페이지에서 API 키를 입력해주세요.'
        );
  ...
  const handleEditCategory = async oldName => {
    ...
    try {
      ...
      toast.success(
        `카테고리가 수정되었습니다. ${response.data.updatedPosts}개 게시글이 업데이트되었습니다.`
      );
      await fetchMetadata(); // Refresh metadata
    } catch (error) {
      if (error.response?.status === 409) {
  ...
  const handleDeleteCategory = async categoryName => {
    if (
      !window.confirm(
        `'${categoryName}' 카테고리를 삭제하시겠습니까? 해당 카테고리를 사용하는 모든 게시글은 '기술' 카테고리로 변경됩니다.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setManagedCategories(response.data.categories);
      toast.success(
        `카테고리가 삭제되었습니다. ${response.data.updatedPosts}개 게시글이 '${response.data.replacementCategory}' 카테고리로 변경되었습니다.`
      );
      await fetchMetadata(); // Refresh metadata
    } catch (error) {
  ...
  const uploadImages = async files => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          `${response.data.images.length}개 이미지가 업로드되었습니다.`
        );
    ...
    <div className='container mx-auto px-4 py-8 max-w-7xl'>
      <div className='flex justify-between items-center mb-8'>
        ...
        <div className='flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4'>
          ...
          <button
            onClick={() =>
              setCategoryManager(prev => ({ ...prev, isOpen: true }))
            }
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
          ...
          <div>
            ...
            <select
              ...
              value={post.year}
              onChange={e =>
                setPost(prev => ({ ...prev, year: e.target.value }))
              }
              className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500'
              disabled={isEditing}
            >
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-4'>
          <div>
            ...
            <input
              ...
              value={
                post.publishTime
                  ? moment(post.publishTime).format('YYYY-MM-DDTHH:mm')
              ...
              onChange={e => {
                ...
                setPost(prev => ({
                  ...prev,
                  publishTime: newDateTime
                    ? moment(newDateTime).format('YYYY-MM-DD HH:mm:ss')
          ...
          <div>
            ...
            <input
              ...
              value={post.readTime || ''}
              onChange={e =>
                setPost(prev => ({ ...prev, readTime: e.target.value }))
              }
              className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500'
              placeholder='ex) 5분'
            />
            ...
            <div className='flex justify-between items-center mb-4'>
              <div>
                ...
                <p className='text-sm text-gray-600'>
                  현재 제공자:{' '}
                  {aiConfig.availableProviders[aiConfig.provider]?.name ||
                    aiConfig.provider}
            ...
            <div className='space-y-4'>
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setAiAssistant(prev => ({ ...prev, type: e.target.value }))
                  }
                  className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500'
                >
              ...
              <div className='bg-gray-50 p-4 rounded-lg'>
                ...
                <div className='text-sm text-gray-600 space-y-1'>
                  ...
                  {aiConfig.provider === 'openrouter' &&
                    aiConfig.availableProviders.openrouter?.model && (
                      <div>
                        모델: {aiConfig.availableProviders.openrouter.model}
                      </div>
                    )}
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        if (newValue.trim()) {
                          setTimeout(
                            () => fetchOpenRouterModels(newValue),
                            500
                          );
                        }
                      }}
              ...
              <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4'>
                <div className='flex items-start space-x-2'>
                  ...
                  <div className='text-sm text-yellow-700'>
                    ...
                    영구적인 설정을 위해서는 서버의 환경변수(.env 파일)를
                    수정해야 합니다.
