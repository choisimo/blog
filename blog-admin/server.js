...
// Utility: enforce timeout on async operations
function withTimeout(promiseOrValue, ms, errorMessage = 'Operation timed out') {
  let timeoutId;
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(() => reject(new Error(errorMessage)), ms);
  });
  // Ensure we always work with a Promise, even if a plain value is passed
  const wrapped = Promise.resolve(promiseOrValue);
  return Promise.race([
    wrapped.finally(() => clearTimeout(timeoutId)),
    timeoutPromise,
  ]);
}
...
class AIService {
  static getSessionConfig(sessionId) {
    ...
    return {
      provider:
        sessionConfig.provider || persistentConfig.provider || AI_PROVIDER,
      apiKeys: {
        gemini:
          sessionConfig.apiKeys?.gemini ||
          persistentConfig.apiKeys?.gemini ||
          GOOGLE_AI_API_KEY ||
          '',
        openrouter:
          sessionConfig.apiKeys?.openrouter ||
          persistentConfig.apiKeys?.openrouter ||
          OPENROUTER_API_KEY ||
          '',
      },
      models: {
        openrouter:
          sessionConfig.models?.openrouter ||
          persistentConfig.models?.openrouter ||
          OPENROUTER_MODEL,
      },
    };
  }

  static setSessionConfig(sessionId, config, makePersistent = true) {
  ...
  static async generateWithGemini(
    ...
    try {
      ...
      if (
        sessionConfig.apiKeys?.gemini &&
        sessionConfig.apiKeys.gemini !== GOOGLE_AI_API_KEY
      ) {
      ...
      const response = await withTimeout(
        result.response,
        Math.min(10000, AI_REQUEST_TIMEOUT_MS),
        'Gemini response processing timed out'
      );
  ...
  static async generateWithOpenRouter(
    prompt,
    type,
    currentContent,
    title,
    ...
    try {
      ...
      const response = await axios.post(
        ...
        },
        {
          headers: {
            ...
            'HTTP-Referer':
              process.env.ADMIN_BACKEND_URL || `http://localhost:${PORT}`,
            'X-Title': 'Blog Admin AI Assistant',
          },
          timeout: AI_REQUEST_TIMEOUT_MS,
          // Enable server-side cancellation when client disconnects
      ...
      if (error.response?.status === 404) {
        throw new Error(
          `Model '${model}' not found on OpenRouter. Please check if the model exists and is accessible.`
        );
      }
      ...
      // 기타 오류
      throw new Error(
        `OpenRouter API 호출에 실패했습니다: ${error.response?.data?.error?.message || error.message}`
      );
    }
  }
...
function generateSlug(title) {
  ...
  if (!slug || slug.trim() === '') {
    throw new Error(
      '제목에 유효한 문자가 없습니다. 알파벳, 숫자, 한글을 포함한 제목을 입력해주세요.'
    );
  }
  ...
  if (slug.length < 2) {
    throw new Error(
      '제목이 처리 후 너무 짧아졌습니다. 더 긴 제목을 입력해주세요.'
    );
  }
  ...
  try {
    ...
    const mdFiles = files.filter(file => {
      ...
      if (file === '.md' || file.startsWith('.md')) {
        console.warn(
          `⚠️  Skipping invalid file during manifest update: ${file}`
        );
        return false;
      }
      ...
      if (file.startsWith('.')) {
        console.warn(
          `⚠️  Skipping hidden file during manifest update: ${file}`
        );
        return false;
      }
    ...
    for (const file of mdFiles) {
      try {
        ...
        if (!content.trim()) {
          console.warn(
            `⚠️  Skipping empty file during manifest update: ${file}`
          );
          continue;
        }
        ...
        console.error(
          `❌ Error validating file ${file} during manifest update:`,
          fileError
        );
    ...
    console.log(
      `Updated manifest for ${year}: ${validFiles.length} valid files, ${mdFiles.length - validFiles.length} skipped`
    );

    if (mdFiles.length - validFiles.length > 0) {
      console.warn(
        `⚠️  ${mdFiles.length - validFiles.length} files were excluded from manifest due to validation issues`
      );
    }
  } catch (error) {
...
app.post('/api/posts', async (req, res) => {
  try {
    const { title, content, excerpt, category, tags, year, publishTime } =
      req.body;
    ...
    if (title.trim().length < 2) {
      return res
        .status(400)
        .json({ error: '제목은 최소 2글자 이상 입력해주세요.' });
    }

    if (title.trim().length > 100) {
      return res
        .status(400)
        .json({ error: '제목은 100글자 이하로 입력해주세요.' });
    }
    ...
    if (content.trim().length < 10) {
      return res
        .status(400)
        .json({ error: '내용은 최소 10글자 이상 입력해주세요.' });
    }
    ...
    if (publishTime) {
      ...
      if (publishMoment.isBefore(now)) {
        return res.status(400).json({
          error:
            '새 게시글의 발행 시간은 현재 시간 이후로 설정해야 합니다 (예약 게시).',
        });
      }
    ...
    if (await fs.pathExists(filePath)) {
      return res.status(409).json({
        error:
          '같은 제목의 게시글이 이미 존재합니다. 다른 제목을 사용해주세요.',
      });
    }
    ...
    if (filename === '.md' || filename.startsWith('.md')) {
      return res.status(400).json({
        error:
          '제목에서 생성된 파일명이 올바르지 않습니다. 영문, 숫자, 한글을 포함한 제목을 사용해주세요.',
      });
    }
...
app.post('/api/git/commit', async (req, res) => {
  try {
    ...
    const commitMessage =
      message || `Add new blog post - ${moment().format('YYYY-MM-DD HH:mm')}`;
...
app.post('/api/deploy', async (req, res) => {
  try {
    ...
    const commitMessage =
      message || `새 게시글 자동 배포 - ${moment().format('YYYY-MM-DD HH:mm')}`;
...
app.put('/api/categories/:oldName', async (req, res) => {
  try {
    ...
    if (categories.includes(newName) && newName !== oldName) {
      return res
        .status(409)
        .json({ error: 'Category with new name already exists' });
    }
...
app.post('/api/ai/generate-content', async (req, res) => {
  try {
    ...
    // Attach a request ID for tracing and set up cancellation on client disconnect
    ...
    try {
      ...
      abortController = null; // Environment may not support AbortController
    }
    const onAbort = () => {
      if (!aborted) {
        aborted = true;
        ...
        console.warn(
          `[AI][${requestId}] client disconnected, aborting in-flight work`
        );
      }
    };
    ...
    if (Object.keys(apiKeys).length > 0 || Object.keys(models).length > 0) {
      // Do NOT persist keys set via content generation; session-only override
      AIService.setSessionConfig(sessionId, { apiKeys, models }, false);
    }
    ...
    try {
      ...
      const provider = effectiveConfig.provider || AI_PROVIDER;

      console.log(
        `[AI][${requestId}] start type=${type} provider=${provider} session=${sessionId} promptLen=${prompt.length}`
      );
      ...
      console.error(`[AI][${requestId}] AI generation error:`, aiError);
      // Fallback to template-based generation
      generatedContent = AIService.generateWithTemplate(
        prompt,
        type,
        currentContent,
        title
    ...
    // If client disconnected during processing, do not attempt to write a response
    if (aborted || req.aborted) {
      console.warn(
        `[AI][${requestId}] request aborted before response; not sending body`
      );
      return;
    }
    ...
    console.log(
      `[AI][${requestId}] success type=${type} provider=${provider} duration=${durationMs}ms`
    );
    ...
    // Map timeout-like errors to 504 to help frontend distinguish
    const isTimeout =
      /timed out/i.test(error?.message || '') || error?.code === 'ECONNABORTED';
    res.status(isTimeout ? 504 : 500).json({
      error: isTimeout ? 'AI request timed out' : 'Failed to generate content',
    ...
    const config = {
      ...
      availableProviders: {
        ...
        openrouter: {
          ...
          status: effectiveConfig.apiKeys?.openrouter
            ? 'configured'
            : 'needs-key',
        },
      },
...
app.post('/api/ai/config', async (req, res) => {
  try {
    ...
    // Store configuration (persist only when requested)
    AIService.setSessionConfig(sessionId, newConfig, persistent);
      ...
      config: {
        ...
        availableProviders: {
          ...
          gemini: {
            available: Boolean(effectiveConfig.apiKeys?.gemini),
            name: 'Google Gemini',
            status: effectiveConfig.apiKeys?.gemini
              ? 'configured'
              : 'needs-key',
          },
          openrouter: {
...
app.post('/api/ai/test', async (req, res) => {
  try {
    ...
    if (!provider || provider === 'template') {
      return res.json({
        success: true,
        message: 'Template provider is always available',
      });
    }
    ...
    if (provider === 'openrouter') {
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          testResult = {
            success: false,
            error: `OpenRouter API test failed: ${error.message}`,
          };
        }
      }
    }
...
app.post('/api/images/upload', upload.array('images', 10), async (req, res) => {
  try {
    ...
    console.log(
      `Uploaded ${uploadedImages.length} images for post ${year}/${postSlug}`
    );
