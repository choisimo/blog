...
(() => {
  ...
  class AIMemoPad extends HTMLElement {
    ...
    render() {
      ...
      doc.innerHTML = `
        ...
        <div id="panel" class="panel">
          <div id="drag" class="header">
            ...
            <div id="minimize" class="minimize" aria-label="최소화">−</div>
            <div id="close" class="close" aria-label="닫기">✕</div>
          </div>
          <div id="actionBar" class="action-bar" role="tablist" aria-label="AI Memo 모드 전환">
            <button id="action-memo" type="button" class="action-btn" data-section="memo" role="tab" aria-selected="true">메모</button>
            <button id="action-chat" type="button" class="action-btn" data-section="chat" role="tab" aria-selected="false">Chat</button>
            <button id="action-preview" type="button" class="action-btn" data-section="preview" role="tab" aria-selected="false">미리보기</button>
            <button id="action-dev" type="button" class="action-btn" data-section="dev" role="tab" aria-selected="false">새 버전 제안</button>
            <button id="action-settings" type="button" class="action-btn" data-section="settings" role="tab" aria-selected="false">설정</button>
          </div>
          <div id="memoBody" class="section-body body active" data-section="memo" role="tabpanel" aria-labelledby="action-memo" aria-hidden="false">
            <div class="section">
          ...
          <div id="chatBody" class="section-body body" data-section="chat" role="tabpanel" aria-labelledby="action-chat" aria-hidden="true">
            <div id="chatHistory" class="chat-history">
              <div class="chat-welcome">
                <p>이 글에 대해 궁금한 점을 물어보세요.</p>
              </div>
            </div>
            <div class="chat-input-area">
              <textarea id="chatInput" class="textarea" placeholder="질문을 입력하세요..." rows="1"></textarea>
              <button id="chatSend" class="btn">전송</button>
            </div>
          </div>

          <div id="previewBody" class="section-body body" data-section="preview" role="tabpanel" aria-labelledby="action-preview" aria-hidden="true">
          ...
          <div id="devBody" class="section-body body" data-section="dev" role="tabpanel" aria-labelledby="action-dev" aria-hidden="true">
            <div class="section">
          ...
          <div id="settingsBody" class="section-body body" data-section="settings" role="tabpanel" aria-labelledby="action-settings" aria-hidden="true">
            <div class="section">
      ...
      this.$chatHistory = this.shadowRoot.getElementById('chatHistory');
      this.$chatInput = this.shadowRoot.getElementById('chatInput');
      this.$chatSend = this.shadowRoot.getElementById('chatSend');
    }
      ...
      try {
        ...
        style.textContent = `
          ...
          *, *::before, *::after { box-sizing: border-box; }
          ...
          .panel {
            position: fixed; z-index: 2147483647; right: 16px; bottom: 160px;
          ...
          .header {
            ...
            padding: 14px 18px; border-bottom: 1px solid rgba(15,23,42,0.08);
          ...
          .header .close:hover { background: rgba(15,23,42,0.08); }
          .tabs {
            display: flex; gap: 6px;
            padding: 10px 12px; border-bottom: 1px solid rgba(15,23,42,0.05);
          ...
          .chat-history {
            flex: 1; border: 1px solid rgba(15,23,42,0.08);
            border-radius: 12px; padding: 12px; background: rgba(248,250,252,0.8);
          }
          .chat-input-area { display: flex; gap: 8px; align-items: flex-end; }
          .chat-input-area textarea { min-height: 48px; }
          .footer {
            padding: 14px 18px; border-top: 1px solid rgba(15,23,42,0.08);
          ...
          .history-overlay {
            ...
            display: none; background: rgba(2,6,23,0.6);
          ...
          @media (max-width: 768px) {
            .panel {
              ...
              max-height: calc(92dvh - env(safe-area-inset-bottom, 8px));
            ...
            .tabs {
              top: calc(56px + 1px); /* header height */
              display: grid;
              grid-template-columns: repeat(2, minmax(0, 1fr));
            }
            .tab { flex: 1 1 auto; }
            .body { padding: 14px; }
            .footer { position: sticky; bottom: 0; padding-bottom: calc(14px + env(safe-area-inset-bottom, 8px)); }
          }
        `;
    ...
    restore() {
     ...
     updateMode() {
      ...
      if (this.$chatBody) this.$chatBody.classList.toggle('active', mode === 'chat');
     ...
     bind() {
      ...
      if (this.$minimize) {
        this.$minimize.addEventListener('click', () => {
          ...
          this.$minimize.textContent = this.state.isMinimized ? '+' : '−';
        });
      }
    ...
    async sendChatMessage() {
      ...
      const prevStatus = this.out.getStatus();
      try {
        this.out.setStatus('AI 답변 생성 중…');
        if (this.$chatSend) this.$chatSend.disabled = true;
        if (this.$chatInput) this.$chatInput.disabled = true;

        const backend = window.__APP_CONFIG?.apiBaseUrl || window.APP_CONFIG?.apiBaseUrl || DEFAULT_API_URL;
        const endpoint = `${backend.replace(/\/$/, '')}/api/v1/ai/summarize`; // Reusing summarize endpoint for now
        ...
        const data = await res.json();
        const reply = (data?.data?.summary || data?.summary || '').toString();
        
        this.appendChatMessage('ai', reply);
        this.out.setStatus('완료');
      } catch (err) {
        console.error('Chat error:', err);
        this.appendChatMessage('ai', '죄송합니다. 답변을 생성하는 중에 오류가 발생했습니다.');
        this.out.setStatus('오류');
      } finally {
    ...
    appendChatMessage(role, text) {
      ...
      this.state.chatHistory.push({ role, text, t: Date.now() });
    }
