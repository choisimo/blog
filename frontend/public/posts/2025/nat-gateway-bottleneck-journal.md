...
> **이 글은 수업 시간에 떠올린 속마음을 정리한 창작 에세이입니다.**

![public-private-subnet-nat-diagram](../../images/2025/nat-subnet-vpc.png)
...
같은 키워드가 적히고 있었다. 그런데 내 머릿속에는 완전히 다른 그림이 그려졌다.
...
유체역학 실험에서 봤던 병목 그림과 똑같았다. 그래서 수업을 듣는 와중에, 자연스럽게 이런 문장이 떠올랐다.
...
3. **수학 레이어** – 큐 길이, 처리율(throughput), 지연(latency), 포아송 분포, 리틀의 법칙 같은 것들

슬라이드에는 1번만 있지만, 내 머릿속에서는 2번과 3번이 계속 같이 깜빡였다. "이 추상적인 네모와 동그라미 뒤에는, 어떤 수치와 제한이 숨어 있을까?" 라는 질문이 꼬리를 물고 이어졌다.
...
> 누군가는 "보안 구조"라고 부르는 다이어그램을 보며, 나는 동시에 "병목 구조"를 떠올렸다.

같은 그림을 보면서도 각자 다른 층위를 떠올리는 이 작은 차이가, 언젠가 설계와 디버깅에서 내 강점이 되지 않을까 하는 막연한 기대가 있다. 그리고 그 기대의 출발점이 바로 이 수업 시간의 속마음이었기에, 이렇게 한 편의 글로 남겨 두기로 했다.
