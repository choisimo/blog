...
const { mockNavigate, mockTrackTagClick } = vi.hoisted(() => ({
  mockNavigate: vi.fn(),
  mockTrackTagClick: vi.fn(),
}));
        ...
        postView={
          {
            ...
            readingTimeLabel: '3\r\nmin',
            author: 'Author\u0000Name',
            tagLabels: ['Tag\u0000Label', 'Empty Tag', 'Slash Tag'],
          } as any
        }
        year={'2026\u0000'}
        slug='safe-post'
...
describe('BlogPostHeader', () => {
  it('sanitizes header metadata and tag navigation', () => {
    ...
    expect(screen.queryByText('[Empty Tag]')).not.toBeInTheDocument();
    ...
    // Slashes are valid in query values and must remain encoded during tag navigation.
    fireEvent.click(screen.getByText('[Slash Tag]'));
    expect(mockNavigate).toHaveBeenLastCalledWith('/blog?tag=bad%2Fslash');
  });
});
