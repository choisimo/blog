    ...
    const loadFeatured = async () => {
      try {
        ...
        if (picks.length > 0) {
          const resolved = await Promise.all(
            picks.map(
              async (p) => (await getPostBySlug(p.year, p.slug)) || null,
            ),
          );
    ...
    <div className="container mx-auto px-4 pt-8 pb-28">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button asChild variant="ghost" size="sm">
                <Link
                  to="/blog"
                  className={cn(isTerminal && "hover:text-primary")}
                >
                  View all
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          ...
          <Button asChild variant="ghost" size="sm">
            <Link to="/blog" className={cn(isTerminal && "hover:text-primary")}>
              View all posts
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="min-w-0">
          ...
          >
            {isTerminal ? "$ explore_next" : "Explore Next"}
          </div>
          <div className="text-xs text-muted-foreground">
            {isTerminal
              ? "curated reading directions"
              : "지금 이어서 보기 좋은 흐름"}
          </div>
        </div>
        <ArrowRight
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted-foreground">추천 시작점</span>
          <span
            className={cn(
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted-foreground">문제 해결 흐름</span>
          <span className="text-xs font-mono font-medium text-muted-foreground">
            AI → Web → Infra
          </span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted-foreground">추천 읽기 방식</span>
          <span className="text-xs font-mono font-medium text-muted-foreground">
            AI &amp; ML
          </span>
        </div>
      ...
      >
        {isTerminal
          ? "# no realtime presence exposed"
          : "실시간 접속자 표시는 홈에서 제거되었습니다."}
      </div>
    </div>
  );
}
