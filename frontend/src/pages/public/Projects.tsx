...
const Projects = () => {
  ...
  return (
    ...
    >
      ...
      {error ? (
        <div className='ui-inline-error ui-projects-load-error' role='alert'>
        ...
        <>
          <section
            className='ui-projects-catalog'
            aria-labelledby='project-catalog-title'
            aria-busy={loading}
          >
            <div className='ui-projects-catalog-header'>
              <h2 id='project-catalog-title'>프로젝트 둘러보기</h2>
              <div
                className='ui-projects-view-switch'
                role='group'
                aria-label='프로젝트 표시 방식'
              >
                <button
                  type='button'
            ...
            <div className='ui-projects-discovery'>
              <div className='ui-projects-search'>
                ...
                <input
                  ...
                  maxLength={200}
                  placeholder='이름, 설명, 기술로 검색'
                  aria-describedby='project-results-summary'
                />
                {searchQuery && (
                  <button
                    type='button'
              ...
              <label className='ui-projects-sort'>
                정렬
                <select
                  value={sortMode}
                  onChange={event =>
                    setSortMode(
                      event.target.value === 'title' ? 'title' : 'source'
                    )
                  }
                >
                  <option value='source'>최근 푸시 날짜순</option>
            ...
            <TagFilter
              ...
              label='프로젝트 주제 필터'
            />
            <div className='ui-projects-results-summary'>
              <p
                id='project-results-summary'
                role='status'
                aria-live='polite'
                aria-atomic='true'
              >
                {loading
                  ? '프로젝트를 불러오는 중입니다.'
                  : `${filteredProjects.length}개 저장소${selectedTag !== 'All' ? ` · ${selectedTag}` : ''}`}
              </p>
            ...
            {loading ? (
              ...
              >
                {Array.from({ length: 4 }, (_, index) => (
                  <ProjectCardSkeleton key={index} presentation={viewMode} />
                ))}
              </div>
            ) : !filteredProjects.length ? (
              <div className='ui-projects-empty'>
                <h3>
                  {projects.length
                    ? '조건에 맞는 프로젝트가 없습니다'
                    : '공개된 프로젝트가 없습니다'}
                ...
                <p>
                  {projects.length
                    ? '검색어를 바꾸거나 주제 필터를 해제해 주세요.'
                    : '새 프로젝트가 공개되면 이곳에서 확인할 수 있습니다.'}
                </p>
                {!!projects.length && (
                  <button
                    type='button'
              ...
              >
                {filteredProjects.map(project => (
                  <ProjectCard
                    key={project.id}
                    project={project}
                    title={`최근 푸시: ${project.date}`}
                    onPreview={handlePreview}
                    openExternally={isMobile && project.type === 'embed'}
                    presentation={viewMode}
                    emphasized={project.featured === true}
                    visitLabel='저장소 열기'
                    codeLabel='확인한 소스'
                  />
                ))}
      ...
      <ProjectModal
        ...
        closeLabel='미리보기 닫기'
        unavailableMessage='이 프로젝트의 미리보기를 표시할 수 없습니다. 공개 주소가 있다면 새 탭에서 열어 주세요.'
      />
    </div>
  );
};
