...
describe('ShellModal', () => {
  it('sanitizes path, suggestions, logs, and command history labels', () => {
    ...
    expect(
      screen.getByRole('dialog', { name: '터미널 명령 입력' })
    ).toBeInTheDocument();
    expect(screen.getByTitle('~/posts')).toBeInTheDocument();
    expect(
      screen.getByRole('listbox', { name: '명령어 제안' })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('option', { name: '명령어 제안 선택: help' })
    ...
    fireEvent.click(
      screen.getByRole('button', { name: '명령 기록 실행: find' })
    );
  ...
  it('exposes close, real terminal, and quick command actions', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: '실제 Linux 터미널로 전환' })
    );
    fireEvent.click(
      screen.getByRole('button', { name: '빠른 명령 실행: help' })
    );
  ...
  it('does not render when closed', () => {
    ...
    expect(
      screen.queryByRole('dialog', { name: '터미널 명령 입력' })
    ).not.toBeInTheDocument();
  });

  it('leaves input Escape and Tab to command suggestions while header Escape closes the modal', () => {
    const props = baseProps();
    render(<ShellModal {...props} suggestions={['help']} />);
    const input = screen.getByRole('textbox', { name: '터미널 명령어' });
    input.focus();
