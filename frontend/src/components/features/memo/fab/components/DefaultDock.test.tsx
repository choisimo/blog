...
const TestIcon = forwardRef<SVGSVGElement, SVGProps<SVGSVGElement>>(
  (props, ref) => <svg ref={ref} data-testid='dock-icon' {...props} />
);
...
describe('DefaultDock', () => {
  it('sanitizes mobile action labels and hides decorative icon and badge', () => {
    const { container } = render(
      <DefaultDock
        dockActions={[
          action({ badge: true, title: '\u001b[33mOpen chat\u001b[0m' }),
        ]}
        isMobile
      />
    );
    ...
    expect(screen.getByTestId('dock-icon')).toHaveAttribute(
      'aria-hidden',
      'true'
    );
    expect(
      container.querySelector('.mobile-action-bar__badge')
    ).toHaveAttribute('aria-hidden', 'true');
    expect(
      screen.getByRole('button', { name: '읽기 설정' })
    ).toBeInTheDocument();
  ...
  it('sanitizes desktop labels and exposes disabled state', () => {
    render(
      <DefaultDock
        dockActions={[
          action({ disabled: true, title: '  Stack\nunavailable  ' }),
        ]}
        isMobile={false}
      />
    );
  ...
  it('uses sanitized labels for the left icon-only dock', () => {
    render(
      <DefaultDock
        dockActions={[
          action({ key: 'memo', label: '\u001b[35m메모\u001b[0m' }),
        ]}
        isMobile={false}
        isLeft
      />
