...
const TERMINAL_DOCK_CONTROL_TEXT_PATTERN =
  /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g;

function normalizeTerminalDockLabel(
  value: unknown,
  fallback = 'Action'
): string {
...
export function TerminalDock({
    ...
    return (
      <div className='flex flex-col items-center gap-0.5 border border-border bg-[hsl(var(--terminal-code-bg))] py-2 px-1 backdrop-blur-sm'>
        {dockActions.map(action => {
          ...
          const displayLabel = normalizeTerminalDockLabel(
            action.desktopLabel || action.label,
            action.key
          );
          const title = normalizeTerminalDockLabel(
          ...
          return (
            ...
            >
              <Icon
                aria-hidden='true'
                className={cn('h-4 w-4', action.primary && 'terminal-glow')}
                focusable='false'
              />
              {action.badge && (
                <span
  ...
  return (
    <div className='flex w-auto max-w-full items-center gap-0.5 overflow-hidden border border-border bg-[hsl(var(--terminal-code-bg))] backdrop-blur-sm'>
      ...
      <div
        aria-hidden='true'
        className='flex items-center gap-2 border-r border-border/50 px-3 py-2.5'
      >
      ...
      <div
        aria-hidden='true'
        className='hidden items-center gap-1 border-r border-border/50 px-3 text-[11px] font-mono text-muted-foreground sm:flex'
      >
      ...
      <div
        aria-hidden='true'
        className='hidden items-center gap-2 border-l border-border/50 px-3 text-[10px] font-mono text-foreground/70 sm:flex'
      >
