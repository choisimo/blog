...
export default function ChatWidget(props: { onClose?: () => void }) {
  ...
  const [persistOptIn, setPersistOptIn] = useState<boolean>(() => {
    if (typeof window === 'undefined') return true;
    try {
      const stored = window.localStorage.getItem('ai_chat_persist_optin');
      if (stored == null) {
        window.localStorage.setItem('ai_chat_persist_optin', '1');
        return true;
      }
      return stored === '1';
    } catch {
  ...
  const setPersistStorage = useCallback((next: boolean) => {
  ...
  const togglePersistStorage = useCallback(() => {
    setPersistStorage(!persistOptIn);
  }, [persistOptIn, setPersistStorage]);
        ...
        <div className='flex items-start justify-between border-b bg-background/95 px-4 py-3'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <div className='mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground'>
                ...
                <label className='inline-flex items-center gap-1 text-[11px] sm:text-xs'>
                  <span className='sr-only'>기록 저장 토글</span>
                  <Switch
                    checked={persistOptIn}
                    onCheckedChange={checked => setPersistStorage(!!checked)}
                    aria-label='AI Chat 기록 저장 토글'
                    className='h-5 w-9'
                  />
                </label>
              </div>
          ...
          <div className='flex items-center gap-1'>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align='end'
                className={cn('w-48 text-sm', isMobile && 'w-56 text-base py-2')}
              >
                <DropdownMenuItem
                  disabled={!sessions.length}
