...
describe('ChatDialogs', () => {
  it('sanitizes image drawer labels and disables unsafe image URLs', () => {
    ...
    const safeImage = screen.getByRole('button', {
      name: '이미지 열기: Diagram',
    });
    const unsafeImage = screen.getByRole('button', {
      name: '이미지 열기: Unsafe image',
    });
  ...
  it('sanitizes mobile action sheet labels and closes after running actions', () => {
    ...
    const { container } = render(
      <MobileActionSheet
        ...
        uploadedImages={[
          { id: 'image-1', name: 'Image', url: '/image.png', size: 1 },
        ]}
        persistOptIn={false}
    ...
    expect(
      screen.getByRole('button', { name: '최근 대화 보기, 1개' })
    ).toBeEnabled();
    expect(
      screen.getByRole('button', { name: '이미지 메모 보기, 1개' })
    ).toBeEnabled();
    expect(debateButton).toHaveAttribute(
      'title',
      '현재 방 AI 토론 (dev), LIVE'
    );
    expect(container.textContent).not.toContain('\u001b');
  ...
  it('keeps unavailable mobile actions disabled', () => {
    ...
    expect(
      screen.getByRole('button', { name: '최근 대화 보기, 0개' })
    ).toBeDisabled();
    expect(
      screen.getByRole('button', { name: '이미지 메모 보기, 0개' })
    ).toBeDisabled();
    expect(
      screen.getByRole('button', { name: '현재 방 AI 토론, LIVE' })
    ).toBeDisabled();
  });
});
