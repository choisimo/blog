...
export function HomeEditorPicksSection({
  ...
  return (
    <section className="ui-home__section ui-editor-picks" aria-labelledby="home-picks-title">
      <PageHeader level={2} id="home-picks-title" title={isTerminal ? '// editor_picks' : "Editor's Picks"}
        actions={<Link to="/blog" className="ui-inline-link">전체 보기</Link>} />
      {sanitizedNotice && <ContentStatus kind="notice">{sanitizedNotice}</ContentStatus>}
      {state === 'loading' ? <ContentStatus kind="loading">추천 글을 불러오는 중입니다.</ContentStatus>
        : editorPickItems.length === 0 ? <ContentStatus kind={state === 'error' ? 'error' : 'empty'}>{state === 'error' ? '추천 글을 불러오지 못했습니다.' : '추천 포스트를 준비 중입니다.'}</ContentStatus>
          : <div className="ui-picks-list">
            {editorPickItems.map(({ post, year, slug, title, category, readingTime }, index) => (
              <Link key={`${year}/${slug}`} to={`/blog/${year}/${slug}`}
                className={`ui-pick${index === 0 ? ' ui-pick--lead' : ''}${post.coverImage ? ' ui-pick--image' : ''}`}>
                <article className="ui-pick__body">
                  <div className="ui-post-meta"><span>{category}</span><span>{formatDate(post.date)}</span>{readingTime && <span>{readingTime}</span>}</div>
                  <h3 className="ui-pick__title">{title}</h3>
                  {index === 0 && (post.excerpt || post.description) && <p className="ui-pick__excerpt">{sanitizeDisplayText(post.excerpt || post.description)}</p>}
                </article>
                {post.coverImage && <div className="ui-pick__cover"><OptimizedImage src={post.coverImage} alt={title} className="ui-pick__image" /></div>}
              </Link>
            ))}
          </div>}
    </section>
