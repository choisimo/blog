...
export function HomeLatestPostsSection({
  ...
  return (
    <section className="ui-home__section" aria-labelledby="home-latest-title">
      <PageHeader level={2} id="home-latest-title" title={isTerminal ? '// latest_posts' : 'Latest Posts'}
        actions={<Link to="/blog" className="ui-inline-link">전체 보기</Link>} />
      <div className="ui-home-reading-grid">
        <div className="ui-latest-list">
          {state === 'error' ? <ContentStatus kind="error">{safeError}</ContentStatus>
            : state === 'loading' ? <ContentStatus kind="loading">최신 글을 불러오는 중입니다.</ContentStatus>
              : safePosts.length === 0 ? <ContentStatus kind="empty">아직 공개된 글이 없습니다.</ContentStatus>
                : safePosts.map(post => (
                  <Link key={`${post.year}/${post.slug}`} to={`/blog/${post.year}/${post.slug}`}
                    className={`ui-post-row${post.coverImage ? ' ui-post-row--image' : ''}`}>
                    <article>
                      <div className="ui-post-meta"><span>{post.category}</span><span>{formatDate(post.date)}</span>{post.readingTime && <span>{post.readingTime}</span>}</div>
                      <h3 className="ui-post-row__title">{post.title}</h3>
                      {post.description && <p className="ui-post-row__description">{post.description}</p>}
                    </article>
                    {post.coverImage && <div className="ui-post-row__cover"><OptimizedImage src={post.coverImage} alt={post.title} className="ui-pick__image" /></div>}
                  </Link>
                ))}
        </div>
        <aside className="ui-home-rail" aria-labelledby="home-tags-title">
          <h3 id="home-tags-title">Popular Tags</h3>
          {safeTags.length === 0 ? <p className="ui-home-rail__notice">표시할 태그가 없습니다.</p>
            : <div className="ui-topic-list">{safeTags.slice(0, 6).map(tag => (
              <Link key={tag.name} to={`/blog?tag=${encodeURIComponent(tag.name)}`} className="ui-topic-link">
                <span>#{tag.name}</span><span className="ui-topic-count">{tag.count}</span>
              </Link>
            ))}</div>}
        </aside>
      </div>
