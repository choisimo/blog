...
const PostCard = memo(
  ({
    ...
    const { bookmarked, toggleBookmark } = useIsBookmarked(
      `${post.year}/${post.slug}`,
    );
    ...
    const showSwipeBookmarkHint = showBookmark && swipeProgress > 0.3;
    const showBookmarkAction = showBookmark && !showSwipeBookmarkHint;
    ...
    const readingTimeLabel = useMemo(() => {
      const raw =
        post.readingTime || (post.readTime ? `${post.readTime} min` : "");
      if (!raw) return "";
    ...
    if (variant === "list") {
      return (
        <article
          ...
          className={cn(
            "group relative flex gap-4 p-4 rounded-xl border border-border/50 bg-card transition-all",
            "hover:shadow-lg hover:border-primary/30",
            isTerminal &&
              "border-border bg-[hsl(var(--terminal-code-bg))] hover:border-primary hover:shadow-[0_0_15px_hsl(var(--primary)/0.2)]",
        ...
        >
          ...
          {showSwipeBookmarkHint && (
            <div className="absolute left-2 top-1/2 z-10 flex -translate-y-1/2 items-center gap-1 rounded-lg bg-card/90 px-2 py-1 text-xs font-medium text-primary">
    ...
    return (
      <article
        ...
        className={cn(
          ...
          isTerminal &&
            "rounded-lg border-border bg-[hsl(var(--terminal-code-bg))] hover:border-primary hover:shadow-[0_0_20px_hsl(var(--primary)/0.3)]",
      ...
      >
        ...
        <Link
          ...
          className={cn(
            ...
            isFeatured
              ? "md:w-3/5 aspect-[16/9] md:aspect-auto"
              : "aspect-video",
          )}
        >
          {post.coverImage ? (
          ...
          <div className="absolute top-3 left-3">
            <Badge
              className={cn(
                "text-xs shadow-sm",
                getCategoryColor(post.category),
                isTerminal && "rounded font-mono border-none",
              )}
            >
              {isTerminal ? `[${post.category}]` : post.category}
            </Badge>
              ...
              {post.tags.slice(0, 3).map((tag) => (
                <Badge
                  ...
                  className={cn(
                    ...
                    isTerminal &&
                      "rounded border-primary/40 text-primary font-mono",
                  )}
                >
                  {isTerminal ? `#${tag}` : `#${tag}`}
                </Badge>
