...
export class PostService {
  ...
  private static parseMarkdownContent(
    ...
    > = {};
    const declaredAvailableLanguages = Array.isArray(
      frontMatter.availableLanguages
    )
      ? frontMatter.availableLanguages.filter(
          (lang: unknown): lang is SupportedLanguage =>
            lang === 'ko' || lang === 'en'
        )
      : [];
