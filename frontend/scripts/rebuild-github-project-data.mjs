...
const publicRepos = inventory.filter(repo =>
  repo.nameWithOwner.startsWith('choisimo/') &&
  repo.isPrivate === false &&
  repo.visibility === 'PUBLIC'
);
if (
  !publicRepos.length ||
  new Set(publicRepos.map(repo => repo.nameWithOwner)).size !== publicRepos.length
) {
...
const entries = publicRepos.map(repo => {
  ...
  if (
    ...
    (proof.commitStatus === 'ok' &&
      (proof.rootStatus !== 'ok' || !['ok', 'missing'].includes(proof.readmeStatus)))
  ) {
  ...
  const source = {
    ...
    head: proof.head ?? null,
    readmeStatus: proof.readmeStatus,
    languages,
  };
...
const catalog = {
  account: 'choisimo',
  checkedAt: entries.map(entry => entry.source.checkedAt).sort().at(-1),
  ...
  counts: {
    ...
    original: publicRepos.filter(repo => !repo.isFork).length,
    forks: publicRepos.filter(repo => repo.isFork).length,
    empty: publicRepos.filter(repo => repo.isEmpty).length,
  },
...
if (checkOnly) {
  if (currentFiles.length !== entries.length) {
    throw new Error('Project Markdown count differs from the public inventory.');
  }
  for (const entry of entries) {
    ...
    if (JSON.stringify(parsed) !== JSON.stringify(entry.project)) {
      throw new Error(`Frontmatter changed meaning: ${entry.filename}`);
    }
  }
  if (JSON.stringify(readJSON(catalogPath)) !== JSON.stringify(catalog)) {
    throw new Error('Public provenance catalog is stale.');
  }
  const manifest = readJSON(path.join(frontendDir, 'public/projects-manifest.json'));
  if (
    manifest.total !== entries.length ||
    manifest.items.length !== entries.length ||
    new Set(manifest.items.map(item => item.id)).size !== entries.length
  ) {
    throw new Error('Manifest has missing or duplicate entries.');
  }
  for (const entry of entries) {
    ...
    if (
      !item ||
      Object.entries(expected).some(([key, value]) => JSON.stringify(item[key]) !== JSON.stringify(value))
    ) {
      throw new Error(`Manifest differs from verified source: ${entry.filename}`);
    }
  }
  console.log(
    `Verified all ${entries.length} public repositories: ${catalog.counts.original} originals, ${catalog.counts.forks} forks, ${catalog.counts.empty} empty. No missing, extra or duplicate project entries.`
  );
} else {
  fs.mkdirSync(projectDataDir, { recursive: true });
  for (const file of currentFiles) fs.unlinkSync(file);
  for (const entry of entries) {
    fs.writeFileSync(path.join(projectDataDir, entry.filename), entry.markdown);
  }
  fs.writeFileSync(catalogPath, `${JSON.stringify(catalog, null, 2)}\n`);
  console.log(
    `Replaced ${currentFiles.length} project sources with ${entries.length} reviewed public repository entries. Run npm run generate-projects-manifest next.`
  );
}
