...
| Worker | 도메인 | 역할 | 상태 |
|--------|--------|------|------|
| **blog-api-gateway** | `api.example.com` | 통합 API Gateway (인증, 콘텐츠, AI, 분석) | ✅ Active |
| **r2-gateway** | `assets.example.com` | R2 스토리지 접근 제어 및 캐싱 | ✅ Active |
| **terminal-gateway** | `terminal.example.com` | WebSocket 터미널 프록시 | ✅ Active |
...
flowchart LR
    ...
    subgraph "assets.example.com"
        R2_GW{R2 Gateway}
        PUB[Public Assets]
        INT[Internal API]
    end
    
    subgraph "terminal.example.com"
...
| api-gateway | `API_BASE_URL` | `https://api.example.com` | secret 권장 |
| r2-gateway | `ALLOWED_ORIGINS` | `*` | `blog.example.com,...` |
| terminal-gateway | `TERMINAL_ORIGIN` | `https://terminal-origin.example.com` | 동일 |
...
| 서비스 | 도메인 | 설명 |
|--------|--------|------|
| API Gateway | `api.example.com` | 모든 API 요청 |
| R2 Gateway | `assets.example.com` | 정적 에셋 |
| Terminal Gateway | `terminal.example.com` | WebSocket 터미널 |
| Frontend | `blog.example.com` | React SPA |
| Backend | `origin.example.com` | Node.js 서버 |
...
6. [ ] Health check: `curl https://api.example.com/health`
