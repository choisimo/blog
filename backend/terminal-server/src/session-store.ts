...
export interface SessionStore {
  ...
  close(): Promise<void>;
  claimSession(session: StoredTerminalSession, ttlSeconds: number): Promise<{ ok: true } | { ok: false; reason: string }>;
  markConnected(sessionId: string, userId: string, containerName: string, ttlSeconds: number): Promise<void>;
...
class InMemorySessionStore implements SessionStore {
  ...
  async claimSession(session: StoredTerminalSession): Promise<{ ok: true } | { ok: false; reason: string }> {
...
class RedisSessionStore implements SessionStore {
  ...
  async claimSession(session: StoredTerminalSession, ttlSeconds: number): Promise<{ ok: true } | { ok: false; reason: string }> {
  ...
  async markConnected(sessionId: string, userId: string, containerName: string, ttlSeconds: number): Promise<void> {
  ...
  async listSessions(): Promise<StoredTerminalSession[]> {
    ...
    return values.filter(Boolean).map((value: string) => JSON.parse(value) as StoredTerminalSession);
  }
