...
const PISTON_BASE_URL = process.env.PISTON_URL || INTERNAL_SERVICES.PISTON_URL;
  ...
  try {
    ...
    if (!response.ok) {
      return res
        ...
        .json({ ok: false, error: "Piston runtimes unavailable" });
    }
    ...
    return res
      .status(503)
      .json({ ok: false, error: "Code execution service unavailable" });
  }
});
...
router.post("/", validateBody(executeBodySchema), async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res
        .status(response.status)
        .json({ ok: false, error: data?.message || "Execution failed" });
    }
