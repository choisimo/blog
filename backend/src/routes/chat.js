...
function normalizeQuizType(value) {
  ...
  if (
    ["fill_blank", "multiple_choice", "transform", "explain"].includes(
      normalized,
    )
  ) {
    return normalized;
...
function scheduleAutoRoomReply({
  room,
  triggerSessionId,
  triggerName,
  triggerText,
...
async function ensureSessionNotebook(sessionId, options = {}) {
...
async function reinforceSessionNotebook(
  session,
  userMessage,
  assistantMessage,
) {
...
function getLiveContextForSession(sessionId) {
...
function deriveUserQuery(parts, fallback) {
...
async function resolveMessageContexts({ userQuery, session, enableRag }) {
