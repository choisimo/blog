...
export async function fetchRemoteManifest() {
  ...
  try {
    ...
    console.error('[PostsRepo] Failed to fetch remote manifest:', err.message);
...
export async function fetchRemotePost(year, slug) {
  ...
  try {
    ...
    console.error(`[PostsRepo] Failed to fetch post ${year}/${slug}:`, err.message);
    return null;
  }
}
...
export async function createPost(data) {
  const { title, slug: slugRaw, year: yearRaw, content, frontmatter } = data;
  ...
  if (!validateFilename(filename)) {
    throw new Error('invalid slug/filename');
  }
...
export async function updatePost(year, slug, data) {
  ...
  const { markdown, frontmatter, content } = data;
  let newMd;
