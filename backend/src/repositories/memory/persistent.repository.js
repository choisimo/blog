/**
 * Persistent Memory Repository - database-backed long-term storage
 * 
...
class PersistentMemoryStore {
  ...
  async save(memory) {
    try {
      ...
      await execute(
        ...
        id,
        memory.sessionId,
        memory.type || MEMORY.DEFAULT_TYPE,
        memory.content,
  ...
  async getMemories(sessionId, options = {}) {
    try {
      ...
      const { type, limit = MEMORY.DEFAULT_LIMIT } = options;
  ...
  async search(searchQuery, options = {}) {
    try {
      ...
      const { sessionId, limit = MEMORY.SEARCH_LIMIT } = options;
  ...
  _fallbackGet(sessionId, options = {}) {
    ...
    return filtered.slice(0, options.limit || MEMORY.DEFAULT_LIMIT).reverse();
  }
}
