...
A local run of all 102 existing Playwright cases against the production build (system Chromium, four workers) produced 79 passed, 6 skipped, and 17 failed. Diagnosis identified stale E2E assumptions: the supplied anonymous token did not satisfy the new continuity claims, the <=640px notebook now intentionally fills the viewport, and the mobile reading toolbar becomes inert while scrolling downward. The test fixture now uses the existing anonymous claim contract with a two-day expiry; external requests remain blocked. Geometry checks cover the full-screen mobile and docked/tablet layouts. TOC cases wait for downward concealment and scroll upward before interacting, preserving active-section and focus checks.

The targeted follow-up passed 17 of 18 cases; its remaining scroll-event race was fixed by awaiting the concealed state, and that case passed on rerun. Six article screenshot artifacts regenerated by the passing cases are included. The following GitHub browser run passed 101 of 102 cases; the remaining live-chat test also supplied a legacy anonymous credential. Its fixture response now carries the required claims and a matching userId. A local production build with the CI feature/API settings passed the incoming-live-message case (no skip). The final GitHub run remains the authoritative complete browser-suite result.
